 ...
 */
    ...
    class LoadinAnimation extends HTMLElement {
      constructor() {
        ...
        this.dataThreshold = this.dataset.loadinAnimationThreshold
          ? this.dataset.loadinAnimationThreshold.replace(/\s+/g, "")
          : "";
        ...
        const thresholdValues = this.dataThreshold
          ? this.setThresholdValues(this.dataThreshold)
          : defaultThreshold;
      ...
      setThresholdValues(item) {
        ...
        let filteredThresholds = thresholdArr.filter((thresholdItem) =>
          this.thresholds.hasOwnProperty(thresholdItem),
        );

        // avoids empty array (created by passing invalid values through this.dataThreshold)
        filteredThresholds =
          filteredThresholds.length > 0 ? filteredThresholds : ["0"];

        // gets the value from obj' key-val pair
        const thresholdVal = filteredThresholds.map(
          (item) => this.thresholds[item],
        );
      ...
      init() {
        window.eight.armada.elementRegistry.register({
          key: "loadin-animation",
          assetPath: "/assets/engine-loadin-animation.min.js",
        });
      }
