  ...
  (() => {
    ...
    class StickyProductForm extends HTMLElement {
      ...
      init() {
        ...
        const selectWrap = this.querySelector(
        ...
        const form = this.querySelector(
          '[data-name="basic-product-form"] form',
        );
      ...
      handleVisibility() {
        const stickyFormObserverCallback = (itemsToObserve) => {
          ...
          if (
            !itemsToObserve[0].isIntersecting &&
            itemsToObserve[0].boundingClientRect.top < 0
          ) {
      ...
      handleCustomProperties(e) {
        ...
        if (prop && this.stickyForm) {
          const existingNode = this.stickyForm.querySelector(
            `[name="${prop.name}"]`,
          );
          ...
          if (isSelect) {
            ...
            const getMatchingOption = prop.querySelector(
              `option[value="${selectedOption}"]`,
            );
            if (getMatchingOption)
              getMatchingOption.setAttribute("selected", "");
          }
      ...
      get product() {
        ...
        return window.eight.safeJSONParse(
          this.childForm.getAttribute("product"),
        );
      }
