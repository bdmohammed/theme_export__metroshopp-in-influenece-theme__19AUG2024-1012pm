  ...
  (() => {
    ...
    class FilterForm extends HTMLElement {
      constructor() {
        ...
        this.activeFacetCountContainer = document.querySelector(
          '[data-armada-selector="active-facet-count"]',
        );
        this.formUnderlay = document.querySelector(
          '[data-armada-selector="filter_underlay"]',
        );
        this.isSidebarForm = this.classList.contains("sidebar-form");
        this.isSearchPage()
          ? (this.gridSelector = document.querySelector(
      ...
      connectedCallback() {
        ...
        if (this.filterPosition == "sidebar" && this.isSidebarForm)
          this.addTopbarContainer();
        (0,
      ...
      registerElement() {
        window.eight.armada.elementRegistry.register({
          key: "filter-and-sort",
          assetPath: "/assets/engine-filter-and-sort.min.js",
        });
      }
      ...
      handleApplyBtn(isMobile) {
        this.topbarForm = document.querySelector(
          '[data-armada-selector="filter-topbar-form"]',
        );
        this.applyFilterBtn = this.topbarForm.querySelector(
          '[data-armada-selector="apply-filters"]',
        );
        this.currentModal = this.topbarForm.closest(
          'armada-modal[data-modal-id="filter-modal-trigger"]',
        );

        if (!this.applyFilterBtn || !isMobile) return;

        // Make button sticky on mobile
        const observerRoot = this.currentModal.querySelector(
          '[data-armada-selector="modal-container"]',
        );
        const btnWrapper = this.topbarForm.querySelector(
          '[data-armada-selector="filter_button_wrapper"]',
        );
        const btnContainer = this.topbarForm.querySelector(
          '[data-armada-selector="filter_button_container"]',
        );
        ...
        const observerCallback = (entries, observer) => {
          entries.forEach((entry) => {
            ...
            if (entry.isIntersecting) {
              ...
              const modalInner = formWrapper.closest(
      ...
      addTopbarContainer() {
        ...
        const sectionSelector = document.querySelector(
          `#shopify-section-${filterSectionId}`,
        );
      ...
      removeTopbarContainer() {
        const topbarContainer = document.querySelector(
          '[data-armada-selector="sidebar_facets"]',
        );
        if (topbarContainer) {
      ...
      handleDetailToggle() {
        if (this.open) {
          const getOpenEls =
            this.closest("form").querySelectorAll(`details[open]`);
      ...
      onSubmitHandler(event) {
        ...
        this.productCardWrapper = this.gridSelector.querySelector(
          '[data-armada-selector="collection-card-grid"]',
        );
        this.activeFacetContainer = this.querySelector(
          '[data-armada-selector="active_facet_container"]',
        );
        this.loadingState = this.querySelector(
          '[data-armada-selector="facets-loading"]',
        );
        if (this.productCardWrapper)
          this.productCardWrapper.classList.add("animate-pulse");
        if (this.activeFacetContainer)
          this.activeFacetContainer.classList.add("animate-pulse");
        if (this.loadingState && !this.activeFacetContainer)
      ...
      renderSectionFromFetch(url) {
        ...
        if (this.isSearchPage()) {
          const searchGridId = document.querySelector(
            '[data-armada-selector="search-grid-main"]',
          ).dataset.sectionId;
          const collectionFilterId = document.querySelector(
            '[data-armada-selector="section-search-filters"]',
          ).dataset.sectionId;
          ...
          els = [
            document.querySelector('[data-armada-selector="search-grid-main"]'),
          ];
          selectors = ['[data-armada-selector="search-grid-main"]'];
        } else {
          const collectionGridId = document.querySelector(
            '[data-armada-selector="collection-grid-main"]',
          ).dataset.sectionId;
          const collectionFilterId = document.querySelector(
            '[data-armada-selector="section-collection-filters"]',
        ...
        window.eight.sectionsEngine.sectionRenderingEngine.fetchSection(
          ...
          (data) => {
            ...
            const formEls = document.querySelectorAll(
              '[data-armada-selector="filter-main-form"]',
            );
            const topbarCount = document.querySelector(
              '[data-armada-selector="active-facet-count"]',
            );
            const activeFacetSelector =
              '[data-armada-selector="filter-main-form"] .active-facets';
            ...
            if (returnedEls) {
              ...
              returnedEls.map(function (val) {
                const removeDetailsPrefix = val.replace(
                  /data-armada-filter-form=\"/g,
                  "",
                );
              ...
              if (formEls.length) {
                formEls.forEach((formEl, index) => {
                  ...
                  const facetEl = containerHTML.querySelector(
                    `${activeFacetSelector}`,
                  );
                  let facetContainer = formEl.querySelector(
                    `${activeFacetSelector}`,
                  );
                  let facetContainerSidebar = document.querySelector(
                    '[data-armada-selector="sidebar_facets"] .active-facets',
                  );
                  ...
                  const getCounts = containerHTML.querySelectorAll(
                    `${filterCountSelector}`,
                  );
                  const countContainers = formEl.querySelectorAll(
                    `${filterCountSelector}`,
                  );
                  ...
                  const inputLists = containerHTML.querySelectorAll(
                    `${inputSelector}`,
                  );
                  const inputContainers = formEl.querySelectorAll(
                    `${inputSelector}`,
                  );
            ...
            if (this.activeFacetContainer)
              this.activeFacetContainer.classList.remove("animate-pulse");
            if (
              this.loadingState &&
              !this.loadingState.classList.contains("hidden")
            )
      ...
      getSections() {
        return [
          {
            section:
              document.querySelector("collection-grid").dataset.sectionId,
          },
        ];
      }
    ...
    class FacetRemove extends HTMLElement {
      constructor() {
        super();
        this.filterForm =
          this.closest("facet-filter-form") ||
          document.querySelector("facet-filter-form");
        this.clearAllBtn = this.querySelector(
          '[data-armada-selector="filters_clear_all"]',
        );
        this.formUnderlay = document.querySelector(
          '[data-armada-selector="filter_underlay"]',
        );
        this.isPriceFilter = this.dataset.priceFilterActive ? true : false;
        this.priceFilterMax = this.filterForm.querySelector(
          '[data-armada-selector="filter_price_to"]',
        );
        this.priceFilterMin = this.filterForm.querySelector(
          '[data-armada-selector="filter_price_from"]',
        );

        if (this.filterForm)
          this.filterPosition = this.filterForm.getAttribute("filter-position");
      }
      ...
      connectedCallback() {
        this.querySelector("a").addEventListener("click", (event) => {
          ...
          const form =
            event.target.closest("facet-filter-form") ||
            document.querySelector("facet-filter-form");
    ...
    class SortForm extends HTMLElement {
      constructor() {
        ...
        this.hiddenSortInputs = document.querySelectorAll(
          'facet-filter-form [data-armada-selector="sort_by_value"]',
        );
        this.sortByForm = this.querySelector(
          '[data-armada-selector="sort_by_form"]',
        );
        ...
        if (this.hiddenSortInputs.length) {
          ...
          this.sortByForm.addEventListener("change", () => {
            ...
            this.hiddenSortInputs.forEach((input) => {
              input.value =
                this.sortByForm.querySelector("input:checked").value;
            });
