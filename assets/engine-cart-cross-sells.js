...
/******/ (() => {
  ...
  (() => {
    ...
    class CartCrossSells extends HTMLElement {
      ...
      connectedCallback() {
        this.renderEl = this.querySelector(
      ...
      init(listeners = false) {
        ...
        const crossSellSection =
          this.cartWrap.querySelector("cart-cross-sells");
        if (crossSellSection) {
      ...
      setListeners() {
        ...
        document.addEventListener("ARMADA:SECTION:FETCH", (e) =>
          this.sectionFetchHandler(e.detail),
        );

        const debouncedResizeHandler = (0,
        ...
        if (dynamic) {
          const debouncedMutationHandler = (0,
          _weareeight_armada_dist_utils_debounce__WEBPACK_IMPORTED_MODULE_1__.debounce)(
            this.dynamicButtonsUpdated,
            500,
          );
      ...
      fetchRecs() {
        ...
        if (
          Object.keys(window.eight.cartEngine.globalCart.highest).length === 0
        ) {
        ...
        if (
          ...
          window.eight.cartEngine.globalCart.highest.product_id
        ) {
          this.previousHighest =
            window.eight.cartEngine.globalCart.highest.product_id;
          const selector =
      ...
      showFallback() {
        const fallbacks = this.querySelectorAll(
      ...
      showTitle() {
        this.querySelector(
          '[data-armada-selector="cart-cross-sells-heading"]',
      ...
      async sectionFetchHandler({ selectors }) {
        if (!selectors.includes('[data-armada-selector="cart-main-section"]'))
          return;

        // Await images loading before resetting dimensions.
        const images = document.querySelectorAll(
          '[data-armada-selector="cart-main-section"] img',
        );
      ...
      get cartHeight() {
        return this.summary
          ? this.cartSection.offsetHeight + this.summary.offsetHeight
          : this.cartSection.offsetHeight;
      }
      ...
      get matchingColorSchemes() {
        ...
        const cartMainSection = document.querySelector(
      ...
      get isDesktop() {
        return (
          window.eight.getCurrentBreakpoint() === "xl" ||
          window.eight.getCurrentBreakpoint() === "2xl"
        );
      }
