...
/******/ (() => {
  ...
  (() => {
    ...
    class Price extends HTMLElement {
      ...
      connectedCallback() {
        this.regularPrice = this.querySelector(
          `[data-armada-selector="price-${this.sectionId}"]`,
        );
        this.compareAt = this.querySelector(
          `[data-armada-selector="compare-at-${this.sectionId}"]`,
        );
        this.discountAmt = this.querySelector(
          `[data-armada-selector="discount-${this.sectionId}"]`,
        );
        this.unitPriceWrap = this.querySelector(
          `[data-armada-selector="unit-price-container-${this.sectionId}"]`,
        );
        this.unitPrice = this.querySelector(
          `[data-armada-selector="unit-price-${this.sectionId}"]`,
        );
        this.unitPriceRef = this.querySelector(
          `[data-armada-selector="unit-price-ref-${this.sectionId}"]`,
        );
        this.compareAtWrap = this.querySelector(
          `[data-armada-selector="compare-at-container"]`,
        );
        this.discountAmtWrap = this.querySelector(
      ...
      init() {
        window.eight.armada.elementRegistry.register({
          key: "product-price",
          assetPath: "/assets/component-price.min.js",
        });
      }
      ...
      attributeChangedCallback(attribute, oldVal, newVal) {
        ...
        switch (attribute) {
          case "price":
            this.regularPrice.innerText =
              window.eight.productEngine.formatMoney(newVal, this.moneyFormat);
            break;
          case "compare-at":
            if (
              newVal === null ||
              newVal === "null" ||
              +newVal < +this.getAttribute("price")
            ) {
              this.compareAt.innerText = "";
              this.compareAtWrap.classList.add("hidden");
            } else {
              this.compareAt.innerText = window.eight.productEngine.formatMoney(
                newVal,
                this.moneyFormat,
              );
          ...
          case "discount":
            if (
              ...
              newVal === "null" ||
              (this.compareAtWrap &&
                this.compareAtWrap.classList.contains("hidden"))
            ) {
          ...
          case "unit-price-measurement":
            ...
            const newMeasurementRef =
              measurement.reference_value !== 1
                ? measurement.reference_value + measurement.reference_unit
                : measurement.reference_unit;
            if (newVal === null || newVal === "null") {
              this.unitPriceWrap.classList.add("hidden");
      ...
      get moneyFormat() {
        return this.getAttribute("money-format")
          ? this.getAttribute("money-format")
          : "${{amount}}";
      }
