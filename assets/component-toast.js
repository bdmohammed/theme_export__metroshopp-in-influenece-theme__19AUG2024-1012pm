    ...
    class Toast extends HTMLElement {
      ...
      init() {
        ...
        window.eight.armada.elementRegistry.register({
          key: "component-toast",
          assetPath: "/assets/component-toast.min.js",
        });
      ...
      addNotification(type, list, id, toastTemplate) {
        if (type != "add") return;
        const notification = list.filter(
      ...
      clearErrors(type) {
        ...
        const errorToasts = this.querySelectorAll(
      ...
      createNewToast(notification, toastTemplate) {
        const newToast = toastTemplate
          .querySelector(`[data-armada-selector="toast-${notification.type}"]`)
          .cloneNode(true);
      ...
      handleAddToCart(e) {
        ...
        const message =
          quantity > 1
            ? `${quantity} ${this.itemsAddedMessage}`
            : `1 ${this.itemAddedMessage}`;
