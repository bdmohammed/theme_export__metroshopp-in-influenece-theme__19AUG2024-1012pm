...
/******/ (() => {
  ...
  (() => {
    ...
    class QuickAdd extends HTMLElement {
      ...
      init() {
        this.currentVariant = window.eight.productEngine.firstAvailableVariant(
          this.product,
        );

        this.templateContent = this.querySelector(
          `template#quick-add-${this.sectionId}-${this.product.id}`,
        ).content;
        this.templateOptionsWrap = this.templateContent.querySelector(
        ...
        window.eight.armada.elementRegistry.register({
          key: "quick-add",
          assetPath: "/assets/components-quick-add.js",
        });
      }

      render() {
        // Keep the passed-in tailwind styles.
        this._classList = this.templateContent.querySelector(
          '[data-name="product-submit"]',
        )
          ? this.templateContent.querySelector('[data-name="product-submit"]')
              .classList
          : "";
        this.buttonWrap = this.querySelector(
          '[data-name="product-submit-wrapper"]',
    ...
    `;
      ...
      buttonHtml(type = "add") {
        ...
        const {
          ...
          isSoldOut,
          chooseVariant,
        } = window.eight.translationEngine.strings;
    ...
    `;
      ...
      handleButtonClick(e) {
        ...
        if (action === "choose") {
          ...
          this.selects = this.querySelectorAll(
            '[data-name="quick-add-options"] select',
          );
      ...
      get selectVals() {
        return this.selects
          ? Array.from(this.selects).map((s) => s.value)
          : false;
      }
