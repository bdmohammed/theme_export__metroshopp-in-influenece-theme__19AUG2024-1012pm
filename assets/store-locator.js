!function() {
  const e = JSON.parse(document.querySelector("[data-store-locator-options]").getAttribute("data-store-locator-options"));
  if (mapboxgl.accessToken = atob(e.accessToken),
  document.addEventListener("shopify:section:load", e => {
      if (![...e.target.classList].includes("main-store-locator"))
          return;
  ...
  "not4s" == mapboxgl.accessToken)
      return;
  const t = JSON.parse(document.getElementById(`stores-json${e.sid}`).textContent)
    , o = new mapboxgl.Map({
      container: `map${e.sid}`,
      style: {
  ...
  if (o.addControl(new mapboxgl.NavigationControl),
  e.enableSearchBox) {
      const e = new MapboxGeocoder({
          ...
          render: function(e) {
              ...
              return '<div class="mapboxgl-ctrl-geocoder--suggestion"><div class="mapboxgl-ctrl-geocoder--suggestion-title needsclick">' + t[0] + '</div><div class="mapboxgl-ctrl-geocoder--suggestion-address needsclick">' + t.splice(1, t.length).join(",") + "</div></div>"
          }
      });
  ...
  o.on("load", () => {
      ...
      function(t) {
          ...
          for (const e of t.features) {
              ...
              s.addEventListener("click", function(e) {
                  e.preventDefault();
                  for (const e of t.features)
                      this.id === `listing-${e.properties.id}` && (n(e),
                      a(e));
      ...
      function() {
          for (const e of t.features) {
              ...
              s || (t.innerHTML = '<svg class="icon icon--store-locator" aria-hidden="true" focusable="false" role="presentation"><use href="#icon--store-locator-marker"></use></svg>'),
              new mapboxgl.Marker(t,{
                  offset: [0, -23]
              }).setLngLat(e.geometry.coordinates).addTo(o),
  ...
  function a(e) {
      ...
      const {name: s, content: n} = e.properties
        , a = new mapboxgl.Popup({
          closeOnClick: !1
      }).setLngLat(e.geometry.coordinates).setHTML(`<h3>${s}</h3>${n}`).addTo(o);
      a._container.querySelector(".mapboxgl-popup-content p").classList.add("rte"),
...
(() => {
  ...
  const storeLocatorOptions = JSON.parse(optionsElement.getAttribute("data-store-locator-options"));
  ...
  const storeData = JSON.parse(document.getElementById(`stores-json${storeLocatorOptions.sid}`).textContent);
  ...
  map.on("load", () => {
    ...
    const renderListings = (data) => {
      const listingsContainer = document.getElementById(`listings${storeLocatorOptions.sid}`);
  ...
  const highlightListing = (feature) => {
    ...
    const listingElement = document.getElementById(`listing-${feature.properties.id}`);
    ...
    const popup = new mapboxgl.Popup({ closeOnClick: false })
      ...
      .setHTML(`<h3>${feature.properties.name}</h3>${feature.properties.content}`)
      .addTo(map);

    const popupContent = popup._container.querySelector(".mapboxgl-popup-content p");
