  ...
  (() => {
    ...
    class productBadges extends HTMLElement {
      ...
      init() {
        ...
        this.allBadges = Object.assign({}, fixedBadges, {
          ...this.customBadges,
        });
        this.badges = [];
        ...
        this.limit = window.eight.badgeEngine
          ...
          : 1;
        const order = window.eight.badgeEngine.order.map((badge) =>
          badge.type === "badge_custom" ? badge.text : badge.type,
        );
        ...
        order.forEach((key) => {
          if (Object.keys(this.allBadges).includes(key)) {
            let badgeObject = window.eight.badgeEngine.order.find(
              (raw) => raw.text === key || raw.type === key,
            );

            if (badgeObject) {
              // handle metafield value
              if (badgeObject.type === "badge_custom") {
                badgeObject = Object.assign({}, badgeObject, {
                  text: this.customBadges[badgeObject.text],
                });
              }
      ...
      renderBadges() {
        ...
        this.badges.forEach((badge, index) => {
          ...
          if (index + 1 <= this.limit) {
            ...
            const badgeElement = this.appendChild(
            ...
            const span = badgeElement.appendChild(
    ...
    class badgePreview extends HTMLElement {
      ...
      handleBlockSelect(e) {
        ...
        const getMatchingBlock = this.querySelector(
