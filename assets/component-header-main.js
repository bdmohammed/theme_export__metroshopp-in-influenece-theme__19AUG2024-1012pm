...
/******/ (() => {
  ...
  (() => {
    ...
    class HeaderMain extends HTMLElement {
      constructor() {
        super();
        this.navigationToggle = this.querySelector(
          '[data-armada-selector="navigation-toggle"]',
        );
        this.navigation = this.querySelector(
          '[data-armada-selector="main-navigation"]',
        );
        this.mobileMenuOpenIcon = this.querySelector(
          '[data-armada-selector="mobile-menu-open-icon"]',
        );
        this.mobileMenuCloseIcon = this.querySelector(
          '[data-armada-selector="mobile-menu-close-icon"]',
        );
        this.localeSelectors = this.querySelector(
          '[data-armada-selector="header-locale-selectors"]',
        );
        this.searchToggles = this.querySelectorAll(
      ...
      init() {
        ...
        document.addEventListener(
          ...
          this.openMobileNavigation.bind(this),
        );
        document.addEventListener(
        ...
        document.addEventListener(
          "ARMADA:SEARCH:TOGGLE",
          this.triggerSearchToggle.bind(this),
        );
      }
      ...
      handleSearchToggle(e) {
        if (e.type === "click" || e.keyCode === 13) {
          ...
          const searchResults = document.querySelector(
            '[data-armada-selector="predictive-search-results"]',
          );
          const searchBlocks = document.querySelector(
            '[data-armada-selector="predictive-search-blocks"]',
          );
          ...
          searchBar.classList.contains("h-0", "opacity-0", "invisible")
      ...
      handleNavDropdown(e) {
        if (
          e.target.closest('[data-armada-selector="navigation-item-details"]')
        )
          return;
      ...
      toggleMobileNavigation() {
        const isOpen =
          this.navigationToggle.getAttribute("aria-expanded") === "true";
        if (isOpen) {
          window.eight.eventsEngine.emit(
            `${eight.constants.events["ARMADA:NAVIGATION:MOBILE:CLOSE"]}`,
          );
          this.navigationToggle.setAttribute("aria-expanded", "false");
          this.navigationToggle.setAttribute("aria-label", "Open navigation");
        } else {
          window.eight.eventsEngine.emit(
            `${eight.constants.events["ARMADA:NAVIGATION:MOBILE:OPEN"]}`,
          );
      ...
      buildLocaleSelectors() {
        ...
        const navAnimationCount = this.navigation.querySelectorAll(
          ".animate-menu-right",
        ).length;
        if (navAnimationCount < 10) {
          localeSelectorsClone.classList.add(
            "animate-menu-right",
            `animate-menu-right-delay-${navAnimationCount + 1}`,
          );
        }
