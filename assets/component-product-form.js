    ...
    class ProductForm extends HTMLElement {
      ...
      init() {
        ...
        this.buttons = this.querySelectorAll(
          '[data-name="product-submit"][data-product-form-block="true"]',
        );
        this.buttonWraps = this.querySelectorAll(
          '[data-name="product-submit-wrapper"][data-product-form-block="true"]',
        );
        ...
        this.checkboxWrapper = this.querySelector(
          "gift-card-recipient-form [data-checkbox-wrapper]",
        );
        this.giftCardRecipientCheckbox = this.querySelector(
          "gift-card-recipient-form #show_gift_card_recipient_checkbox",
        );
        this.giftCardRecipientInputFieldsWrapper = this.querySelector(
          "gift-card-recipient-form .gift_card_recipient_fields",
        );
        this.giftCardRecipientInputFields = this.querySelectorAll(
          'gift-card-recipient-form [name*="Recipient"]',
        );
      ...
      setPickup() {
        const pickupElem = document.querySelector(
        ...
        this.currentVariant.options.forEach((option, index) => {
          if (this.optionType === "dropdowns") {
            ...
            this.optionInputs.forEach((input) => {
              // Case insensitive.
              if (
                input.value.localeCompare(option, undefined, {
                  sensitivity: "base",
                }) === 0
              ) {
                input.checked = true;
      ...
      setDynamicFormVariantId() {
        const dynamicFormIds = this.querySelectorAll(
      ...
      handleOptionChange(e) {
        ...
        if (
          ...
          this.currentVariant.title !== prevTitle
        ) {
          window.eight.eventsEngine.namedEvents.changeVariant(
            this.product.id,
      ...
      handleButtonClick(e) {
        ...
        const productProperties = this.querySelectorAll(
          '[name^=properties]:not([type="checkbox"]):not([type="radio"]):not([disabled]), [name^=properties][type="checkbox"]:not([disabled]):checked, [name^=properties][type="radio"]:not([disabled]):checked',
        );
        const properties = {};
      ...
      updateFormVariant(variantId) {
        ...
        const variantIdInputs = this.querySelectorAll(
          "#add-to-cart-form input[data-armada-variant-id]",
        );
      ...
      atcSuccess(data) {
        ...
        if (this.giftCardRecipientCheckbox.checked) {
          ...
          this.giftCardRecipientInputFieldsWrapper.classList.add(
            "invisible",
            "opacity-0",
          );
      ...
      setButton(mode = false, reset = false) {
        ...
        if (!mode) {
          ...
          if (this.forceSelection && !this.allOptionsChosen) {
            ...
            this.buttons.forEach((btn) => {
              btn.ariaLabel =
                window.eight.translationEngine.strings.chooseVariant;
            ...
            this.buttons.forEach((btn) => {
              btn.ariaLabel =
                window.eight.translationEngine.strings.isUnavailable;
            });
          ...
          this._buttons.forEach((_btn) => {
            ...
            _btn.ariaLabel =
              window.eight.translationEngine.strings.successfully;
            _btn.disabled = true;
      ...
      get radioGroups() {
        ...
        for (let index = 1; index <= this.product.options.length; index++) {
          const elems = this.querySelectorAll(
            `[name="${this.sectionId}-product-${this.product.id}-option-${index}"]`,
          );
      ...
      get optionVals() {
        ...
        if (this.optionType === "dropdowns") {
          ...
          this.radioGroups.forEach((group, index) => {
            const checkedVal = Array.from(group)
              .filter((s) => s.checked)
              .map((s) => s.value);
      ...
      get blockOrder() {
        const order = this.getAttribute("block-order")
      ...
      get blocks() {
        ...
        const blocks = this.blockOrder.map((blockName) => {
          return this.templateContent.querySelector(
            `[data-name="${blockName}"]`,
          );
        });
