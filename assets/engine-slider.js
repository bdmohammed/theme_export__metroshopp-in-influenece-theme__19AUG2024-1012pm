...
/******/ (() => {
  ...
  (() => {
    ...
    /* harmony import */ var _weareeight_armada_dist_utils_debounce__WEBPACK_IMPORTED_MODULE_1__ =
      __webpack_require__(273);
    /* harmony import */ var _weareeight_armada_dist_utils_debounce__WEBPACK_IMPORTED_MODULE_1___default =
      /*#__PURE__*/ __webpack_require__.n(
        _weareeight_armada_dist_utils_debounce__WEBPACK_IMPORTED_MODULE_1__,
      );
    ...
    class sliderEngine extends HTMLElement {
      ...
      init() {
        ...
        if (this.isVertical && this.sliderFrame.innerHeight <= this.stageHeight)
          return;
        if (this.isHorizontal && this.sliderFrame.innerWidth <= this.stageWidth)
          return;
      ...
      render() {
        if (this.arrows && this.templateContent) {
          const prevIcon = this.templateContent
            .querySelector('[data-name="icon-prev"]')
            .cloneNode(true);
          const nextIcon = this.templateContent
        ...
        if (this.counter && this.templateContent && !navExists) {
          const counterElems = this.templateContent
        ...
        const indicatorExists = this.querySelector(
          '[data-name="mobile-indicator"]',
        );

        if (this.mobileIndicator && this.templateContent && !indicatorExists) {
          const mobileIndicator = this.templateContent.querySelector(
            '[data-name="mobile-indicator"]',
          );
      ...
      attachEvents() {
        ...
        const returnedFunction = (0,
        _weareeight_armada_dist_utils_debounce__WEBPACK_IMPORTED_MODULE_1__.debounce)(
          this.resizeHandler,
          250,
        );
        window.addEventListener("resize", returnedFunction);

        this.sliderWrap.addEventListener("touchstart", this.touchstartHandler, {
          passive: true,
        });
        this.sliderWrap.addEventListener("touchend", this.touchendHandler, {
          passive: true,
        });
        this.sliderWrap.addEventListener("touchmove", this.touchmoveHandler, {
          passive: true,
        });
        ...
        if (this.arrows) {
          if (this.nextArrows) {
            this.nextArrows.forEach((next) =>
              next.addEventListener("click", this.nextArrowHandler),
            );
          ...
          if (this.prevArrows) {
            this.prevArrows.forEach((prev) =>
              prev.addEventListener("click", this.prevArrowHandler),
            );
      ...
      detachEvents() {
        ...
        if (this.nextArrows)
          this.nextArrows.forEach((next) =>
            next.removeEventListener("click", this.nextArrowHandler),
          );
        if (this.prevArrows)
          this.prevArrows.forEach((prev) =>
            prev.removeEventListener("click", this.prevArrowHandler),
          );
      ...
      setSlideDimensions() {
        if (this.totalSlides > this.visibleSlides && this.visibleSlides > 0) {
          ...
          if (this.loop) {
            const clones = this.sliderFrame.querySelectorAll(
              '[data-cloned="true"]',
            );
      ...
      setNavAndProgressWidth() {
        this.slideElems.forEach((slideElem) => {
          ...
          const slideContents = slideElem.querySelector(
            '[data-name="slide-content"]',
          );
      ...
      blockSelectHandler(e) {
        const slideIndex = this.slideElems.findIndex(
      ...
      handleVariantChange(event) {
        ...
        const mediaId = variant.featured_media
      ...
      slideToMediaId(mediaId, disableAnimation = false) {
        ...
        this.currentSlide = this.slideElems.findIndex(
          (slide) => parseInt(slide.getAttribute("media-id")) == mediaId,
        );
      ...
      slideVisible(direction = false, reEnableTransition = false) {
        ...
        if (this.isVertical) {
          ...
          if ((direction == "next" && !atEnd) || direction == "previous") {
            this.sliderFrame.style["transform"] =
      ...
      getOffset(direction) {
        ...
        const currentOffset = this.isVertical
          ? currentTransform.y
          : currentTransform.x;
        let offset = 0;
          ...
          if (isVisible) {
            ...
            if (direction == "next" && slideElem.nextElementSibling) {
              ...
              if (slideElem.previousElementSibling) {
                newBounds =
                  slideElem.previousElementSibling.getBoundingClientRect();
      ...
      getEventDirection(e) {
        ...
        if (this.isVertical) {
          direction =
            e.clientY - window.innerHeight / 2 > 0 ? "previous" : "next";
        } else {
          direction =
            e.clientX - window.innerWidth / 2 > 0 ? "previous" : "next";
        }
      ...
      isVisible(element, full = false) {
        ...
        if (!full) {
          ...
          if (
            this.isVertical &&
            elBounds.top >= stageBounds.top &&
            elBounds.bottom <= stageBounds.bottom
          ) {
            return true;
          } else if (
            this.isHorizontal &&
            elBounds.left >= stageBounds.left &&
            elBounds.right <= stageBounds.right
          ) {
            return true;
          }
      ...
      previewNextSlide() {
        ...
        const currentSlideOffset =
          -1 * this.currentSlide * (this.stageWidth / this.visibleSlides);
        const nextSlideOffset =
          -1 * (this.currentSlide + 1) * (this.stageWidth / this.visibleSlides);
        const previewDecimal = 0.5; // Proportion of next slide to preview
        const previewOffset =
          (nextSlideOffset + currentSlideOffset) * previewDecimal;
      ...
      scrollBarsItemCount() {
        ...
        const moves =
          this.visibleSlides > 1 || this.visibleSlides.sm > 1 || this.isMobile
        ...
        const trackIndicator =
          this.scrollBars[0].querySelector(".scroll-range");
      ...
      onChange() {
        ...
        const currentSlide =
          this.visibleSlides === 1 ? this.currentSlide : false;
        ...
        if (this.progressBars && currentSlide !== false) {
          ...
          const barForThisSlide =
            this.slideElems[currentSlide].querySelector("progress-bar");
          ...
          if (barForThisSlide) {
            ...
            const thisBar =
              this.progressBars[currentSlide].querySelector(
                '[data-name="bar"]',
              );

            if (thisBar) {
              if (
                this.slideElems[currentSlide].dataset.slideType === "slide_link"
              ) {
        ...
        if (this.scheme && this.counterWraps.length) {
          ...
          this.counterWraps.forEach((wrap) =>
      ...
      setAutoHeight() {
        ...
        const activeSlideContent =
          currentSlideElem.querySelector("div:not([hidden])");
        const activeSlideHeight = activeSlideContent.offsetHeight;
        const hasHeight = currentSlideElem.style.height ? true : false;

        if (this.isMobile) {
          this.slideElems.forEach(
            (slideElem) => (slideElem.style.height = `${activeSlideHeight}px`),
          );
        } else if (hasHeight) {
      ...
      resolveVisibleSlides() {
        ...
        if (!visibleSlidesAttr) {
          ...
          const visibleObject = window.eight.safeJSONParse(
          ...
          if (typeof visibleObject === "object") {
            ...
            if (currentBreakpoint !== "") {
              ...
              if (!visibleObject[currentBreakpoint]) {
                const breakpointPxValue =
                  window.eight.getBreakpointValue(currentBreakpoint);

                const nextBreakpoint = Object.keys(visibleObject).reduce(
                  (a, b) => {
      ...
      get featuredImageId() {
        return this.getAttribute("featured-image-id")
      ...
      get isMobile() {
        return (
          ...
          window.eight.getCurrentBreakpoint() === "md"
        );
      }
      ...
      get isAutoHeight() {
        return (
          this.getAttribute("auto-height") === "true" &&
          this.isHorizontal &&
          this.getAttribute("visible-slides") == 1
        );
      }
      ...
      get animationDuration() {
        const animationDurationVar = getComputedStyle(
