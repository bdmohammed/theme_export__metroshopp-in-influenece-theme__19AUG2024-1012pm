    ...
    class Video extends HTMLElement {
      ...
      init() {
        if (!this.isHero) {
          ...
          document.addEventListener(
            "ARMADA:MEDIA:CHANGED",
            this.mediaChanged.bind(this),
          );
        } else {
        ...
        window.eight.armada.elementRegistry.register({
          key: "component-video",
          assetPath: "/assets/component-video.min.js",
        });
      }
      ...
      get isHero() {
        return this.getAttribute("data-is-hero")
          ? this.getAttribute("data-is-hero")
          : false;
      }
        ...
        const callback = (mutationList, observer) => {
          mutationList.forEach((mutation) => {
            if (
              ...
              mutation.attributeName == "loaded"
            ) {
              window.eight.eventsEngine.emit(
                `${eight.constants.events["ARMADA:MEDIA:LOADED"]}`,
              );
              observer.disconnect();
            }
          });
      ...
      watchActiveState(currentVideo) {
        ...
        this.isMobile
          ? (attributeToWatch = "data-slide-active")
          : (attributeToWatch = "data-thumbnail-selected");
        ...
        function callback(mutationList, observer) {
          mutationList.forEach(function (mutation) {
            if (
              mutation.type === "attributes" &&
              mutation.attributeName === attributeToWatch
            ) {
              window.eight.eventsEngine.emit(
                `${eight.constants.events["ARMADA:MEDIA:CHANGED"]}`,
              );
            }
          });
        }
      ...
      mediaChanged() {
        ...
        if (videoParentEl) {
          ...
          this.isMobile
            ...
            : (isActive = videoParentEl.getAttribute(
                "data-thumbnail-selected",
              )); //Returns true or false

          if (
            (!this.isMobile && isActive == "true") ||
            (this.isMobile && isActive)
          ) {
            //Play the video
            this.setVideoState(
              this.type,
              this.getVideoId(this.type, this.url),
              "play",
            );
          } else if (
            (!this.isMobile && isActive == "false") ||
            (this.isMobile && !isActive)
          ) {
            //Pause the video
            this.setVideoState(
      ...
      setVideoState(videoType, videoId, action) {
        ...
        let videoElDesktop = document.querySelector(
          `[data-armada-selector="product-media"] [data-video-id="${videoId}"]`,
        );
        ...
        if (video)
          currentAction = this.getVideoAction(videoType, action, video);
        ...
        if (videoType == "youtube") {
          video.contentWindow.postMessage(
            `{"event":"command","func":"${currentAction}","args":""}`,
            "*",
          );
      ...
      handleMP4Hero(newContent) {
        ...
        newContentSource.addEventListener("error", () =>
          this.handleFallbackImage(newContent),
        );
      }
