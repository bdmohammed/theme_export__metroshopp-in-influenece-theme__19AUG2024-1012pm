  ...
  (() => {
    ...
    class PredictiveSearch extends HTMLElement {
      constructor() {
        ...
        this.initialBlocks = this.querySelector(
          '[data-armada-selector="predictive-search-blocks"]',
        );
        this.activeResults = this.querySelector(
          '[data-armada-selector="predictive-search-results"]',
        );
        this.loadingState = this.querySelector(
          '[data-armada-selector="results-loading"]',
        );
        this.clearSearch = this.querySelector(
          '[data-armada-selector="clear-search"]',
        );
        this.closeIconInitial = this.querySelector(".close-icon-blocks");
        this.input = this.querySelector('input[type="search"]');

        if (this.activeResults)
          this.loadingStateHtml = this.activeResults.innerHTML;
      ...
      attachEvents() {
        ...
        ["click", "keypress"].forEach((evt) => {
          if (this.closeIconInitial)
            this.closeIconInitial.addEventListener(
              evt,
              this.close.bind(this, this.initialBlocks),
            );
          if (this.clearSearch)
        ...
        if (window.Shopify.designMode) {
          document.addEventListener(
            ...
            this.sectionSelectHandler.bind(this),
          );
          document.addEventListener(
            "shopify:section:deselect",
            this.sectionDeselectHandler.bind(this),
          );
        }
      }
          ...
          function (data) {
            ...
            ["click", "keypress"].forEach((evt) => {
              ...
              closeIcon.addEventListener(
                evt,
                this.close.bind(this, this.activeResults),
              );
            });

            // If the result container is hidden, show it
            if (
              this.activeResults.classList.contains(
                "h-0",
                "opacity-0",
                "overflow-hidden",
              )
      ...
      closeSearch(e) {
        const headerCTA = document.querySelector(
          '[data-armada-selector="header-cta-container"]',
        );
        const mobileSearchCTA = document.querySelector(".header-mobile-search");
        if (
          this.closest("search-bar").contains(e?.target) ||
          headerCTA.contains(e?.target) ||
          mobileSearchCTA.contains(e?.target)
        )
          return;
        this.closest("search-bar").classList.add(
          "h-0",
          "opacity-0",
          "invisible",
        );
      ...
      handleSearchClear() {
        ...
        if (this.initialBlocks) {
          this.initialBlocks.classList.remove("hidden");
        }
        if (this.loadingState) this.showEl(this.loadingState);
        if (this.activeResults) this.hideResultContainer(this.activeResults);
      ...
      resetResults() {
        if (this.activeResults)
          this.activeResults.innerHTML = this.loadingStateHtml;
      }
