...
const paths = {
  ...
  liquid: {
    src: ['templates/**/*.liquid', 'sections/**/*.liquid', 'snippets/**/*.liquid', 'layout/**/*.liquid'],
    watch: ['templates/**/*.liquid', 'sections/**/*.liquid', 'snippets/**/*.liquid', 'layout/**/*.liquid']
  }
};
...
function scripts() {
  return gulp.src(paths.scripts.src)
    ...
    .pipe(terser({
      ...
      mangle: {
        reserved: ['$', 'jQuery', 'Shopify', 'theme', 'Cart', 'Product', 'Collection']
      },
      format: {
        comments: /^!/
      }
...
function analyzeCode() {
  ...
  return gulp.src(['assets/*.min.js', 'assets/*.min.css'])
    .pipe(through2.obj(function(file, _, cb) {
      ...
      if (fileName.includes('.js') && size > 100000) {
        bundleStats.recommendations.push(`⚠️  ${fileName} (${sizeKB}KB) is large. Consider code splitting or removing unused code.`);
      }
      if (fileName.includes('.css') && size > 50000) {
        bundleStats.recommendations.push(`⚠️  ${fileName} (${sizeKB}KB) is large. Consider removing unused CSS or splitting into multiple files.`);
      }
    ...
    .on('end', () => {
      // Write bundle stats
      fs.writeFileSync('reports/bundle-stats.json', JSON.stringify(bundleStats, null, 2));
...
function performanceAudit() {
  ...
  console.log('• Use {% render %} instead of {% include %} for better performance');
...
const prod = gulp.series(clean, createReportsDir, lint, compileProd, analyzeCode, performanceAudit);
